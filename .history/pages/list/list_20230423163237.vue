...
<script setup>
	...
	let fabTouchmove = (params) => {
		...
		let x = params.touches[0].pageX + 'rpx'
		let y = params.touches[0].pageY + 'rpx'
		
		let elA = nodeFab.value.$refs.fabA.$el
		let elB = nodeFab.value.$refs.fabB.$el
		// console.log(elA,elB);
		elA.style.transform = `translate(${x},${y})`
		elB.style.transform = `translate(${x},${y})`
		console.log(elA.style.transform,elB.style.transform);
