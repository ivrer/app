...
<script setup>
	...
	let fabTouchmove = (params) => {
		...
		let top = nodeFab.value.$el.offsetTop

		let x = params.touches[0].pageX-200+ 'px'
		let y = params.touches[0].pageY -200 + 'px'

		console.log(nodeFab.value.$el.offsetTop,'offsetTop')
		let elA = nodeFab.value.$refs.fabA.$el
		let elB = nodeFab.value.$refs.fabB.$el
		...
		elA.style.left = x
		elB.style.left = x
		// console.log(window.getComputedStyle(elB).top)
...
.pageView{
 ...
 .fab{
	transform: translate(300rpx,100rpx);
}
</style>