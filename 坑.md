...
###给一个节点添加触动移动时，也会触发页面下拉刷新，也就是说文档中说明的没有事件冒泡，这里也是会发生的,第一次触发后，后面就不会出现了

###app 和小程序是支持跨域的，但网页端调试的时候，虽然是设备是app 但运行在浏览器页面 是H5的受跨域影响
可以解除浏览器的跨域，设置一个支持跨域的浏览器
配置代理服务器，在vue.config.js或manifest。json中，两者只能取其一配置。，这里应用在浏览器的时没有奏效


###异步获取资源信息时，小程序的onready有时会在信息获得之后执行，而H5 web端则会先于，这需要考虑到uni的生命周期